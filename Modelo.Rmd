...
```{r}
library(readxl)
base <- read_excel("C:/Users/HP/Desktop/Tesis UDESA/2026 03 29/base3.xlsx")
...
```{r}
base <- base[, -which(names(base) == "Delayedby_NA")]
base <- base[, -which(names(base) == "Joint_revision")]
#base <- base[, -which(names(base) == "Purchaseschedule_Original")]
```
...
```{r}
base %>% filter(QPC_Criteria_Order == 32) %>% pull(Status) %>% summary()
filtered_table <- table(base$Status[base$QPC_Criteria_Order == 32])
sum(filtered_table)
filtered_table
print(round(prop.table(filtered_table), 3))
```
...
model1 = randomForest(Status ~ . -QPC_Criteria_Order, 
                       data = training_set,
...
pred.train1 <- predict(model1, newdata = training_set, type = "prob")

# Convert the predicted probabilities to numeric
pred.train1 <- as.numeric(pred.train1[, 1])

# Create the ROC curve using pROC
rocbag <- pROC::roc(training_set$Status, pred.train1)
rocbag
...
```{r}
train_numeric5 <- training_set2[training_set2$QPC_Criteria_Order == 32, ]
train_numeric5$Status <- as.factor(train_numeric5$Status)
summary(train_numeric5$Status)
round(prop.table(table(train_numeric5$Status)), 3)
```
...
model2 = randomForest(Status ~ . -QPC_Criteria_Order, 
                       data = training_set2,
                       ntree = best_params$ntree,
                       mtry = best_params$mtry,
                       importance = TRUE,
...
# Predict probabilities using the bagged model
pred.train2 <- predict(model2, newdata = training_set2, type = "prob")

# Convert the predicted probabilities to numeric
pred.train2 <- as.numeric(pred.train2[, 1])

# Create the ROC curve using pROC
rocbag <- pROC::roc(training_set2$Status, pred.train2)
rocbag
...
# Predict probabilities using the bagged model
pred.test <- predict(model2, newdata = test_set_GRA, type = "prob")

# Convert the predicted probabilities to numeric
pred.test <- as.numeric(pred.test[, 1])

# Create the ROC curve using pROC
rocbag <- pROC::roc(test_set_GRA$Status, pred.test)
rocbag
...
pred.bag <- predict(model1, newdata = training_set, type = "prob")
pred.bag <- as.numeric(pred.bag[, 1])
roc_bag <- pROC::roc(training_set$Status, pred.bag)


ggroc(list(`Train Inicial` = roc_bag), 
      size = 1.2, alpha = 0.8) +
  ...
  theme(legend.position = c(0.85, 0.45),
        ...
        axis.line = element_line(color = "grey", size = 0.8)) +
  scale_color_manual(values = c("Train Inicial" = "#3A5F7F")) +
  annotate("text", x = 0.25, y = 0.15, 
           label = paste("Train Inicial AUC:", round(auc(roc_bag), 3)), 
           color = "#3A5F7F", hjust = 0, size = 3.5)
...
pred.rf <- predict(model2, newdata = training_set2, type = "prob")
pred.rf <- as.numeric(pred.rf[, 1])
roc_rf <- pROC::roc(training_set2$Status, pred.rf)
...
ggroc(list(`Train Inicial` = roc_bag, 
  ...
  annotate("text", x = 0.25, y = 0.15, 
           label = paste("Train Inicial AUC:", round(auc(roc_bag), 3)), 
           color = "#3A5F7F", hjust = 0, size = 3.5) +
  annotate("text", x = 0.25, y = 0.08, 
           label = paste("Train Final AUC:", round(auc(roc_rf), 3)), 
           color = "#A83232", hjust = 0, size = 3.5) 
...
pred.test <- predict(model2, newdata = test_set_GRA, type = "prob")
pred.test <- as.numeric(pred.test[, 1])
roc_test <- pROC::roc(test_set_GRA$Status, pred.test)
...
ggroc(list(`Train Inicial` = roc_bag, 
  ...
  annotate("text", x = 0.25, y = 0.08, 
           label = paste("Train Final AUC:", round(auc(roc_rf), 3)), 
           color = "#A83232", hjust = 0, size = 3.5) +
  annotate("text", x = 0.25, y = 0.01, 
           label = paste("Test AUC:", round(auc(roc_test), 3)), 
           color = "darkgrey", hjust = 0, size = 3.5)
```
